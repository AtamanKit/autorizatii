...
class ProgAnual extends React.Component {
  ...
  HandleOF = (varOf) => {
    fetch(`http://localhost:5000/prog-anual/${varOf}/`, {
        method: 'GET',
      })
        .then(res =>  res.json())
        .then(result => this.setState({prog_anual_items: result}))
      ...
      return (
        <React.Fragment>
          ...
          <table className='centerAL'>
            <thead>
              ...
              <tr>
                <th colSpan='8'  className="th-al-title">
                  DECONECTARI ANUALE PROGRAMATE
                </th>
              </tr>
              {this.state.prog_anual_items.length === 0
            ...
            <tbody>
              {this.state.prog_anual_items.map((item) => 
