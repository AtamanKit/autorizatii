...
// import OficiiChoose from './oficii-choose';
...
class NeprLunar extends React.Component {
  ...
  HandleOfMth = (ofVar) => {
    fetch(`http://localhost:5000/nepr-lunar/${ofVar}/${undefined}/`, {
      method: 'GET',
    })
    .then(res => res.json())
    .then(result => this.setState({nepr_lunar_items: result.reverse()}))
  ...
  componentDidMount(){
    ...
    if (document.location.pathname.slice(-2) === "ar") {
      this.HandleOfMth(myCookies[2])
    } else {
      this.HandleOfMth(document.location.pathname.slice(-2))
    }
  }

    // ChooseFunc = (evt, data) => {
    //   OficiiChoose(this.state.nepr_lunar_items, data.value.abr)
    // }
  ...
  ChMthFunc = (evt, {value}) => {
    ...
    fetch(`http://localhost:5000/nepr-lunar/${valueOf}/${value}/`, {
        method: 'GET',
    })
    .then(res =>  res.json())
    .then(result => this.setState({nepr_lunar_items: result.reverse()}))
  ...
  ExcelFunc = () => {
    ...
    fetch(`http://127.0.0.1:5000/nepr-lunar/excel/${this.state.mth_select}/`, {
      method: 'GET',
    })
    ...
    if (this.state.excel_control.status === 403) {
      return(
        <ErrorLoad
          ...
          erBtn="Deconectari neprogramate lunare"
        />
      )
      ...
      return (
        <React.Fragment>
          ...
          <table className='centerAL'>
            <thead>
              ...
              <tr>
                <th colSpan='14'  className="th-al-title">
                  DECONECTARI LUNARE NEPROGRAMATE
                </th>
              </tr>
              {this.state.nepr_lunar_items.length === 0
              ? <Loader size='big' active/>
              :  <tr>
            ...
            <tbody>
              {this.state.nepr_lunar_items.map((item) => 
